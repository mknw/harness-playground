...
- **Auth** — every public action and `/api/events` / `/api/stash` route authenticates via Stack Auth and scopes session ops by `user.id`. In dev with `isBypassEnabled()` (gated on `import.meta.env.DEV && VITE_DEV_BYPASS_AUTH === 'true'`, see `lib/auth/dev-bypass.ts`), the user id falls back to the shared `dev-bypass-user` literal — same value used by the client-side mock so frontend/backend agree.
